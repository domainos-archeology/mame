...
/*
 * apollo_dn300_kbd.c - Apollo keyboard and mouse emulation
 *
...
#define LOG(x)  { m_device->logerror ("%s apollo_dn300_kbd: ", m_device->cpu_context()); m_device->logerror x; m_device->logerror ("\n"); }
...
DEFINE_DEVICE_TYPE(APOLLO_DN300_KBD, apollo_dn300_kbd_device, "apollo_dn300_kbd", "Apollo Keyboard")

//-------------------------------------------------
// apollo_dn300_kbd_device - constructor
//-------------------------------------------------

apollo_dn300_kbd_device::apollo_dn300_kbd_device(const machine_config &mconfig, const char *tag, device_t *owner, uint32_t clock)
	: device_t(mconfig, APOLLO_DN300_KBD, tag, owner, clock)
...
void apollo_dn300_kbd_device::device_add_mconfig(machine_config &config)
{
	/* keyboard beeper */
...
ioport_constructor apollo_dn300_kbd_device::device_input_ports() const
{
	return INPUT_PORTS_NAME(apollo_dn300_kbd);
}
...
void apollo_dn300_kbd_device::device_start()
{
	m_device = this;
	LOG1(("start apollo_dn300_kbd"));
...
void apollo_dn300_kbd_device::device_reset()
{
	LOG1(("reset apollo_dn300_kbd"));
...
std::string apollo_dn300_kbd_device::cpu_context() const
{
...
void apollo_dn300_kbd_device::logerror(Format &&fmt, Params &&... args) const
{
...
apollo_dn300_kbd_device::beeper::beeper() :
	m_device(nullptr),
	m_beeper(nullptr),
	m_timer(nullptr)
{
...
void apollo_dn300_kbd_device::beeper::start(apollo_dn300_kbd_device *device)
{
	m_device = device;
	LOG2(("start apollo_dn300_kbd::beeper"));
	m_beeper = m_device->m_beep.target();
	m_timer = m_device->timer_alloc(FUNC(apollo_dn300_kbd_device::beeper::beeper_callback), this);
}

void apollo_dn300_kbd_device::beeper::reset()
{
	LOG2(("reset apollo_dn300_kbd::beeper"));
	on();
}

void apollo_dn300_kbd_device::beeper::off()
{
	m_beeper->set_state(0);
}

void apollo_dn300_kbd_device::beeper::on()
{
...
int apollo_dn300_kbd_device::beeper::keyboard_has_beeper()
{
	return true;    // driver has no facility to return false here, so go with it
}

TIMER_CALLBACK_MEMBER(apollo_dn300_kbd_device::beeper::beeper_callback)
{
	off();
}
...
apollo_dn300_kbd_device::mouse::mouse() :
	m_device(nullptr)
{
}

void apollo_dn300_kbd_device::mouse::start(apollo_dn300_kbd_device *device)
{
	m_device = device;
	LOG2(("start apollo_dn300_kbd::mouse"));
}

void apollo_dn300_kbd_device::mouse::reset()
{
	LOG2(("reset apollo_dn300_kbd::mouse"));
...
void apollo_dn300_kbd_device::mouse::read_mouse()
{
	if (m_tx_pending > 0)
	{
...
void apollo_dn300_kbd_device::putdata(const uint8_t *data, int data_length)
{
...
void apollo_dn300_kbd_device::putstring(const char *data)
{
	putdata((uint8_t *) data, strlen(data));
}

void apollo_dn300_kbd_device::kgetchar(uint8_t data)
{
...
int apollo_dn300_kbd_device::push_scancode(uint8_t code, uint8_t repeat)
{
	int n_chars = 0;
	uint16_t key_code = 0;
...
TIMER_CALLBACK_MEMBER(apollo_dn300_kbd_device::kbd_scan_timer)
{
	scan_keyboard();
...
apollo_dn300_kbd_device::code_entry const apollo_dn300_kbd_device::s_code_table[] = {
		...
		/* C7      Y           */ { 0x32, 0xB2, 0x59,     0x59,   0x19,   0x59,     NOP,     No  },
...
// Apollo US keyboards have no hash key (#)
		...
		/* E2      Z           */ { 0x60, 0xE0, 0x5A,     0x5A,   0x1A,   0x5A,     NOP,     No  },
