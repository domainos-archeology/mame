...
public:
	...
	template <typename... T>
	void set_emmu_translate_callback(T &&...args) { m_emmu_translate_callback.set(std::forward<T>(args)...); }
...
protected:
	...
	bool m_has_emmu;				/* Indicates if an external MMU is available (apollo DN300 custom 68010) */
